...
@interface LinearSystem (PrivateMethods)                   // Private method interface for LinearSystem object.
...
-(id)initWithContentsOfString:(NSString *) stringContents
{
    ...
    if (self)
    {
        ...
        for (NSUInteger row = 1; row <= _n; row++)
        {
            ...
            [_matrixA insertObject:rowOfValues
                           atIndex:(row-1)];
...
-(void)SolveLinearSystem
{
    ...
    for (NSUInteger step = 0; step < _n; step++)
    {
        ...
        if (pivotElementRowIndexForCurrentStep != step)
        {
            // Framework provided method that swaps the object indices (instead of object contents).
            // Swap rows in matrix A then swap rows in matrix B.
            [_matrixA exchangeObjectAtIndex:step withObjectAtIndex:pivotElementRowIndexForCurrentStep];
            [_matrixB exchangeObjectAtIndex:step withObjectAtIndex:pivotElementRowIndexForCurrentStep];
        }
...
-(void)SaveSolutionToFile
{
    ...
    NSString* stringToSave = [[NSString alloc] init];   // Allocates & initializes a string that will hold our solution values.
    ...
    for (NSUInteger colIndex = 0; colIndex < _n; colIndex++)
    {
        ...
        [_matrixB replaceObjectAtIndex:colIndex
                            withObject:[NSString stringWithFormat:@"%.4f",[[_matrixB objectAtIndex:colIndex] doubleValue]]];
...
-(void)ScaleRowsByLargestAbsoluteValueInRow
{
    for (NSUInteger rowIndex = 0; rowIndex < _n; rowIndex++)
    {
        ...
        for (NSUInteger colIndex = 0; colIndex < _n; colIndex++)
        {
            ...
            [[_matrixA objectAtIndex:rowIndex] replaceObjectAtIndex:colIndex
                                                         withObject:[NSNumber numberWithDouble:temporaryValueInRow]];
        }
        ...
        [_matrixB replaceObjectAtIndex:rowIndex
                            withObject:[NSNumber numberWithDouble:temporaryValueInRow]];
    }
...
-(void)ReduceRowsForStep:(NSUInteger)step
{
    ...
    for (NSUInteger row = (step+1); row < _n; row ++)
    {
        ...
        {
            currentRowElement = [[[_matrixA objectAtIndex:row] objectAtIndex:col] doubleValue];        // The current row's element for current column
            pivotRowElement   = [[[_matrixA objectAtIndex:step] objectAtIndex:col] doubleValue];       // The pivot row's element for current column.
...
{
    //
    double stepElement = [[[_matrixA objectAtIndex:(_n-1)]objectAtIndex:(_n-1)]doubleValue];
    
    // SAFETY: If the diagonal term for this row is considered "zero"...
    if ( fabs(stepElement) < _LS_ZERO_THRESHOLD)
    {
        // Display error message to terminal and terminate the program.
    ...
    double bValue = [[_matrixB objectAtIndex:(_n-1)]doubleValue];
    double aValue = [[[_matrixA objectAtIndex:(_n-1)]objectAtIndex:(_n-1)]doubleValue];
    double xValue = bValue / aValue;
    
    // Wraps double value in NSNumber object wrapper & begins replacing the values in matrix B starting with the last value.
    [_matrixB replaceObjectAtIndex:(_n-1)
                        withObject:[NSNumber numberWithDouble:xValue]];
    ...
    for (NSInteger rowIndex = (_n-2); rowIndex >= 0; rowIndex--)
    {
        ...
        //
        stepElement = [[[_matrixA objectAtIndex:rowIndex]objectAtIndex:rowIndex]doubleValue];
        ...
        bValue = [[_matrixB objectAtIndex:rowIndex]doubleValue];
        aValue = [[[_matrixA objectAtIndex:rowIndex]objectAtIndex:rowIndex]doubleValue];
        xValue = (bValue - rowSum) / aValue;
